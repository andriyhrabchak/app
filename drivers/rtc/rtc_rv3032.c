...
#define RV3032_REG_SECONDS              0x00
#define RV3032_REG_MINUTES              0x01
#define RV3032_REG_HOURS                0x02
#define RV3032_REG_WEEKDAY              0x03
#define RV3032_REG_DATE                 0x04
#define RV3032_REG_MONTH                0x05
#define RV3032_REG_YEAR                 0x06
#define RV3032_REG_ALARM_MINUTES        0x07
#define RV3032_REG_ALARM_HOURS          0x08
#define RV3032_REG_ALARM_WEEKDAY        0x09
#define RV3032_REG_STATUS               0x0E
#define RV3032_REG_CONTROL1             0x0F
#define RV3032_REG_CONTROL2             0x10
#define RV3032_REG_EVENT_CONTROL        0x13
#define RV3032_REG_TS_COUNT             0x14
#define RV3032_REG_TS_SECONDS           0x15
#define RV3032_REG_TS_MINUTES           0x16
#define RV3032_REG_TS_HOURS             0x17
#define RV3032_REG_TS_DATE              0x18
#define RV3032_REG_TS_MONTH             0x19
#define RV3032_REG_TS_YEAR              0x1A
#define RV3032_REG_UNIXTIME0            0x1B
#define RV3032_REG_UNIXTIME1            0x1C
#define RV3032_REG_UNIXTIME2            0x1D
#define RV3032_REG_UNIXTIME3            0x1E
#define RV3032_REG_USER_RAM1            0x1F
#define RV3032_REG_USER_RAM2            0x20
#define RV3032_REG_EEPROM_ADDRESS       0x25
#define RV3032_REG_EEPROM_DATA          0x26
#define RV3032_REG_EEPROM_COMMAND       0x27
#define RV3032_REG_ID                   0x28
#define RV3032_REG_CLKOUT               0x35
#define RV3032_REG_OFFSET               0x36
#define RV3032_REG_BACKUP               0x37

#define RV3032_CONTROL1_TD              BIT(0)
#define RV3032_CONTROL1_TE              GENMASK(2, 1)
#define RV3032_CONTROL1_EERD            BIT(3)
#define RV3032_CONTROL1_USEL            BIT(4)
#define RV3032_CONTROL1_WADA            BIT(5)
#define RV3032_CONTROL1_TRPT            BIT(7)

#define RV3032_CONTROL2_RESET           BIT(0)
#define RV3032_CONTROL2_12_24           BIT(1)
...
#define RV3032_CONTROL2_TSE             BIT(7)

#define RV3032_STATUS_PORF              BIT(0)
#define RV3032_STATUS_EVF               BIT(1)
#define RV3032_STATUS_AF                BIT(2)
#define RV3032_STATUS_TF                BIT(3)
#define RV3032_STATUS_UF                BIT(4)
#define RV3032_STATUS_BSF               BIT(5)
#define RV3032_STATUS_CLKF              BIT(6)
#define RV3032_STATUS_EEBUSY            BIT(7)

#define RV3032_CLKOUT_FD                GENMASK(2, 0)
#define RV3032_CLKOUT_PORIE             BIT(3)
#define RV3032_CLKOUT_CLKSY             BIT(6)
#define RV3032_CLKOUT_CLKOE             BIT(7)

#define RV3032_CLKOUT_FD_LOW            0x7

#define RV3032_BACKUP_TCE               BIT(5)
#define RV3032_BACKUP_TCR               GENMASK(1, 0)
#define RV3032_BACKUP_BSM               GENMASK(3, 2)

#define RV3032_BSM_LEVEL                0x3
...
#define RV3032_ALARM_DATE_AE_WD         BIT(7)
...
static int rv3032_eeprom_wait_busy(const struct device *dev)
{
	...
	for (;;) {
		...
		if (!(status & RV3032_STATUS_EEBUSY)) {
			break;
		}
...
static int rv3032_int_enable_unlocked(const struct device *dev, bool enable)
{
	const struct rv3032_config *config = dev->config;
	uint8_t clkout = 0;
	int err;

	if (enable || config->cof == RV3032_CLKOUT_FD_LOW) {
		/* Disable CLKOUT */
		clkout |= FIELD_PREP(RV3032_CLKOUT_FD, RV3032_CLKOUT_FD_LOW);
	} else {
		/* Configure CLKOUT frequency */
		clkout |= RV3032_CLKOUT_CLKOE |
			  FIELD_PREP(RV3032_CLKOUT_FD, config->cof);
	}

	/* Configure the CLKOUT register */
	err = rv3032_update_cfg(dev,
				RV3032_REG_CLKOUT,
				RV3032_CLKOUT_FD | RV3032_CLKOUT_CLKOE,
				clkout);
	if (err) {
		return err;
	}
...
static void rv3032_work_cb(struct k_work *work)
{
...
#ifdef CONFIG_RTC_ALARM
	if ((status & RV3032_STATUS_AF) && data->alarm_callback != NULL) {
...
#ifdef CONFIG_RTC_UPDATE
	if ((status & RV3032_STATUS_UF) && data->update_callback != NULL) {
	...
	if (((status & RV3032_STATUS_AF) && alarm_callback != NULL) ||
	    ((status & RV3032_STATUS_UF) && update_callback != NULL)) {
...
static int rv3032_get_time(const struct device *dev, struct rtc_time *timeptr)
{
	...
	timeptr->tm_hour = bcd2bin(date[2] & RV3032_HOURS_24H_MASK);
				 ...
				 const struct rtc_time *timeptr)
{
	...
	if (!rv3032_validate_rtc_time(timeptr, mask)) {
	...
	if (mask & RTC_ALARM_TIME_MASK_MONTHDAY) {
		...
		regs[2] = RV3032_ALARM_DATE_AE_WD;
	}

	LOG_DBG("set alarm: mday = %d, hour = %d, min = %d, mask = 0x%04x",
		timeptr->tm_mday, timeptr->tm_hour, timeptr->tm_min, mask);

	/* Write registers RV3032_REG_ALARM_MINUTES through RV3032_REG_ALARM_WEEKDAY */
	return rv3032_write_regs(dev, RV3032_REG_ALARM_MINUTES, &regs, sizeof(regs));
}
				 ...
				 struct rtc_time *timeptr)
{
	...
	*mask = 0U;
	...
	if ((regs[2] & RV3032_ALARM_DATE_AE_WD) == 0) {
				     ...
				     rtc_alarm_callback callback, void *user_data)
{
	...
	err = rv3032_read_reg8(dev, RV3032_REG_CONTROL2, &control_2);
	if (err) {
		goto unlock;
	}

	if (callback != NULL) {
		control_2 |= RV3032_CONTROL2_AIE;
	} else {
		control_2 &= ~(RV3032_CONTROL2_AIE);
	}

	if ((control_2 & RV3032_CONTROL2_UIE) == 0U) {
		/* Only change INT GPIO if periodic time update interrupt not enabled */
		err = rv3032_int_enable_unlocked(dev, callback != NULL);
		if (err) {
			goto unlock;
		}
	}

	err = rv3032_write_reg8(dev, RV3032_REG_CONTROL2, control_2);
	if (err) {
		goto unlock;
	}
				      ...
				      void *user_data)
{
	...
	if (callback != NULL) {
		control_2 |= RV3032_CONTROL2_UIE;
	} else {
		control_2 &= ~(RV3032_CONTROL2_UIE);
	}

	if ((control_2 & RV3032_CONTROL2_AIE) == 0U) {
...
static int rv3032_init(const struct device *dev)
{
	...
	err = rv3032_read_reg8(dev, RV3032_REG_ID, &val);
	if (err) {
		return -ENODEV;
	}
	...
	/* Configure the CLKOUT register */
	val = FIELD_PREP(RV3032_CLKOUT_FD, config->cof) |
	      (config->cof != RV3032_CLKOUT_FD_LOW ? RV3032_CLKOUT_CLKOE : 0);
	err = rv3032_update_cfg(dev,
				RV3032_REG_CLKOUT,
				RV3032_CLKOUT_FD | RV3032_CLKOUT_CLKOE,
				val);
	...
	err = rv3032_update_cfg(dev,
				RV3032_REG_BACKUP,
				RV3032_BACKUP_TCE | RV3032_BACKUP_TCR | RV3032_BACKUP_BSM,
				config->backup);
	if (err) {
		return -ENODEV;
	}

	err = rv3032_update_reg8(dev, RV3032_REG_CONTROL1, RV3032_CONTROL1_WADA,
				 RV3032_CONTROL1_WADA);
	if (err) {
		return -ENODEV;
	}

	/* Disable the alarms */
	err = rv3032_update_reg8(dev,
				 RV3032_REG_CONTROL2,
				 RV3032_CONTROL2_AIE | RV3032_CONTROL2_UIE,
				 0);
	if (err) {
		return -ENODEV;
	}
	...
	err = rv3032_write_regs(dev, RV3032_REG_ALARM_MINUTES, regs, sizeof(regs));
	if (err) {
		return -ENODEV;
	}
...
#define RV3032_BACKUP_FROM_DT_INST(inst)                                                           \
	((FIELD_PREP(RV3032_BACKUP_BSM, RV3032_BSM_FROM_DT_INST(inst))) |                          \
	 (FIELD_PREP(RV3032_BACKUP_TCR, DT_INST_ENUM_IDX_OR(inst, trickle_resistor_ohms, 0))) |    \
	 (DT_INST_NODE_HAS_PROP(inst, trickle_resistor_ohms) ? RV3032_BACKUP_TCE : 0))

#define RV3032_INIT(inst)                                                                          \
	static const struct rv3032_config rv3032_config_##inst = {                                 \
		.i2c = I2C_DT_SPEC_INST_GET(inst),                                                 \
		.cof = DT_INST_ENUM_IDX_OR(inst, clkout_frequency, RV3032_CLKOUT_FD_LOW),          \
